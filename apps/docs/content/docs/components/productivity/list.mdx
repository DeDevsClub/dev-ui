...
import {
  ...
  ListItems,
  ListProvider,
} from '@/components/ui/dedevs-ui/list';
import type { DragEndEvent } from '@/components/ui/dedevs-ui/list';
  ...
  return (
    <ListProvider onDragEnd={handleDragEnd}>
      {exampleStatuses.map((status) => (
        <ListGroup key={status.name} id={status.name} className="bg-primary">
          <ListHeader name={status.name} color={status.color} className="text-white"/>
          <ListItems>
            {features
